...
7. **Tune a speaker (optional).** On a Local Stereo path that renders the audio itself (the system sink or a capture backend), each enabled output row gets a slider-icon button that opens a ten-band graphic equalizer for that speaker alone: 31.5 Hz … 16 kHz, ±12 dB in 0.5 dB steps, plus an overall trim and a bypass switch. Handy when one speaker's bass is overpowering. The curve is stored against that device's CoreAudio UID, so it is re-applied every time the device connects, and it never leaks onto a different display. The same curve applies in AirPlay experimental mode, where each local speaker renders through its own bridge — nothing to re-enter when you switch modes. AirPlay receivers share a single **group** curve instead of one each (OwnTone sends them one stream, so there is nothing per-receiver to shape), offered as its own row in the AirPlay section. Not available on the Direct Stereo path — SyncCast does not render those samples, and the row says so if you have a curve saved.
8. **Widen a small speaker's image (optional).** A compact stereo speaker carries its two tweeters a few centimetres apart above a shared mono woofer, so from a normal seat the two channels arrive at both ears almost equally and the image collapses to a point. On the same paths as the equalizer, each enabled output row gets a second button opening a stereo-image panel with two independent stages: a **width** control that scales the mid/side difference signal above a corner frequency (default 1.5 kHz — below it a speaker like that has no stereo information anyway) and stays mono-compatible by construction; and a **crosstalk cancellation** stage of the recursive kind, whose delay is derived from two sliders you can measure with a ruler — the spacing between the drivers and how far away you sit. The panel shows the delay that falls out of those numbers, and warns how much the recursion boosts centred material, which is the effect's real cost. Both stages sit behind one A/B bypass, because the only way to judge either is to listen from your own seat. Stored against the CoreAudio UID and re-applied on every connect, like the equalizer. Crosstalk cancellation only works from the position it was set up for; step aside and it becomes a comb filter.
9. **Line up two speakers that are out of step (optional).** Some outputs — display panels in particular — add tens of milliseconds of their own audio processing and never report it, so they play noticeably behind the built-in speakers. On the same Local Stereo paths as the equalizer, each enabled output row carries a delay slider (−100…+100 ms, 1 ms ≈ 34 cm of extra distance): positive holds that output back, and only the difference between outputs matters, so the earliest one is always the reference. Devices that *do* report their latency honestly are lined up automatically before you touch anything; the slider covers the rest. Stored against the CoreAudio UID and re-applied on every connect, like the equalizer. No microphone is involved — you dial it in by ear.
10. **Set up auto-connect (optional).** With the devices you want ticked, open 自动连接 under the device list, pick the trigger device and press 「用当前选择创建规则」. From then on that selection is restored whenever the trigger appears — unless you have changed the selection yourself, in which case the rule stands down until the trigger is unplugged and reconnected, or you press 「重新应用规则」.
...
- Per-device ten-band equalizer on every path SyncCast renders itself (both modes), plus one group curve for all AirPlay receivers, remembered by CoreAudio UID
- Per-device stereo image (mid/side width above a corner frequency, plus recursive crosstalk cancellation with the delay derived from driver spacing and listening distance) on the same paths, remembered by CoreAudio UID
