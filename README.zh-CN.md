...
- **每台设备独立调音器 (EQ)**:给每个输出单独一套十段图示均衡器(31.5 Hz … 16 kHz,±12 dB,0.5 dB 一档),外加整体增益微调和旁路开关 —— 某个音箱低音太厉害就单独把它压下来,不影响别的音箱。曲线按 CoreAudio UID 记住,**每次连接自动恢复**,换工位接另一台显示器也不会串。仅在本地 Stereo 且音频真的经过本程序渲染时生效(系统 sink / 捕获路径);Direct Stereo 与 AirPlay 全屋模式下不生效,界面会直接说明而不是给一个动了没反应的滑块。
