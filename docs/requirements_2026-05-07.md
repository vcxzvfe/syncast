...
- Default-output auto-switch/restore: a user-session helper run switched from `多输出设备` to `ExternalDisplay` for `display,xiaomi`, then restored `多输出设备`.
