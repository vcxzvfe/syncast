...
- ✅ **8 devices discovered** by Bonjour + CoreAudio enumeration (Xiaomi, Mac mini, <this Mac's AirPlay Receiver> AirPlay receiver, ExternalDisplay, BlackHole, MBP扬声器, Microsoft Teams Audio, 多输出设备).
...
- **MBP speaker** played
- **Xiaomi Sound** silent
- **ExternalDisplay (display)** silent
- Local sync between MBP + ExternalDisplay was previously audibly off (delay/echo) — fix deployed but unverified
...
**The bug**: When MBP扬声器 + ExternalDisplay are both enabled, the user reported audible delay between them ("听上去很乱").
...
**To verify**: run with `SYNCAST_AUTO_TEST=mbp,display`, play music, listen to whether MBP + ExternalDisplay are now in sync.
...
2. **Xiaomi RTSP timeout**: OwnTone log showed `airplay: Device '<AirPlay receiver B>' closed RTSP connection` once. Xiaomi may close RTSP if buffer underrun happens. Need keep-alive or auto-reconnect logic.
...
Local checkout: /Users/<you>/syncast/
App bundle: /Applications/SyncCast.app/

Read /Users/<you>/syncast/docs/HANDOFF.md for the full state.

Current symptom: after my fixes, MBP speaker plays correctly, but
Xiaomi Sound and ExternalDisplay display speaker fall silent within seconds.
...
2. Whether the multi-output sync fix in LocalOutput.swift actually
   removes the audible delay between MBP and ExternalDisplay
